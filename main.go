...
type model struct {
	...
	key               string
	log               string
	capturingKeyboard bool   // Flag to indicate if keyboard input is being captured
	inputValue        string // Nome della nuova directory
	leftWatcherSub    *fs.DirWatcherSubscription
	rightWatcherSub   *fs.DirWatcherSubscription
}
...
func initialModel() model {
	...
	m.leftWatcherSub, err = fs.SubscribeWatcher(nil, currentDir, func(path string, err error) {
		if err != nil {
			m.log = fmt.Sprintf("Left watcher error: %s", err)
			return
		}
		m.log = fmt.Sprintf("Left watcher: %s", path)
	...
	m.rightWatcherSub, err = fs.SubscribeWatcher(nil, currentDir, func(path string, err error) {
		if err != nil {
			m.log = fmt.Sprintf("Right watcher error: %s", err)
			return
		}
		m.log = fmt.Sprintf("Right watcher: %s", path)
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		if m.capturingKeyboard {

			if key == "enter" {
				if m.confirmAction != nil {
					err := m.confirmAction(&m)
					if err != nil {
		...
		case "ctrl+d":
			...
			m.confirmAction = func(m *model) error {
		...
		case "delete", "ctrl+t":
			...
			m.confirmMessage = fmt.Sprintf("Are you sure you want move to trash\n%s?", strings.Join(paths, "\n"))
			m.confirmAction = func(m *model) error {
		...
		case "ctrl+f":
			...
			m.capturingKeyboard = true
			m.inputValue = ""
			m.confirmMessage = "Enter the name of the new directory:"
			m.confirmAction = func(m *model) error {
				if m.inputValue == "" {
					return fmt.Errorf("Directory name cannot be empty")
				}
				err := m.createDirectory(m.inputValue)
				if err != nil {
		...
		case "ctrl+n":
			...
			m.capturingKeyboard = true
			m.inputValue = ""
			m.confirmMessage = "Enter the name of the new file:"
			m.confirmAction = func(m *model) error {
				if m.inputValue == "" {
					return fmt.Errorf("File name cannot be empty")
				}
				err := m.createFile(m.inputValue)
				if err != nil {
...
func (m *model) enterFile(path string) error {
	...
	if info.IsDir() {
		...
		// Enter directory
		if m.active == "left" {
			newWatcher, err := fs.SubscribeWatcher(m.leftWatcherSub, path, func(eventPath string, err error) {
				if err != nil {
					return
				}
				m.refreshLeftTableRows()
			...
			m.leftWatcherSub = newWatcher
			m.leftPanelDir = path
			m.leftTable = m.leftTable.WithRows(newRows).WithHighlightedRow(0).WithAllRowsDeselected()
			m.leftFilesInfo = filesInfo
		} else {
			newWatcher, err := fs.SubscribeWatcher(m.rightWatcherSub, path, func(path string, err error) {
				if err != nil {
					return
				}
				m.refreshRightTableRows()
...
func (m *model) copyFiles(overwrite bool) error {
	...
	for _, filePath := range paths {
		err := fs.CopyFile(filePath, destPath, false)
		if err != nil {
			return err
		}
	}
...
func (m *model) moveFiles(overwrite bool) error {
	...
	for _, filePath := range paths {
		err := fs.MoveFile(filePath, destPath, false)
		if err != nil {
			return err
		}
	}
...
func (m *model) overlayViews(modal string) string {
...
func (m model) View() string {
	...
	if m.errorMessage != "" {
		return m.alertDialog(m.errorMessage)
	}

	if m.confirmMessage != "" {
		return m.confirmDialog(m.confirmMessage)
	}

	return m.view
}

func (m *model) confirmDialog(text string) string {
	...
	return m.overlayViews(modal)
}

func (m *model) alertDialog(text string) string {
